...
// ─────────────────────────────────────────────────────────────────────────────
// Se ejecuta en GitHub Actions (IPs no bloqueadas por el WAF de los colegios).
// Genera guardias-output.json que el backend de Render lee via Gist público.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── GIPUZKOA: usa la API interna cofgipuzkoa.pretools.net ──────────────────────
...
async function scrapeAlava(page) {
  ...
  try {
    ...
    console.log('🟢 ÁLAVA: Sin selector estándar, extrayendo genéricamente...');
  }
...
// ── BIZKAIA: formulario de municipios en cofbizkaia ───────────────────────────
async function scrapeBizkaia(page) {
  console.log('\n🔴 BIZKAIA: navegando...');

  // Interceptar imágenes/fuentes para acelerar
  ...
  // Sin selector → extraer tabla visible directamente
  if (!selMunicipio) {
...
(async () => {
  ...
  // Ejecutar scrapers secuencialmente (evita consumir demasiada RAM en Actions)
